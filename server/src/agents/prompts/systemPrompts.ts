...
3. "hotel"：用户想查询或预订酒店、民宿、住宿或房间。
4. "weekend"：用户想要规划周末、下午几个小时的连续行程或吃喝玩乐安排，且通常涉及到结界群像偏好。
5. "venue"：用户提到了美团的即时生活服务消费。
6. "chat"：如果上述意图都不明确，分类为日常聊天或神秘学问答。

严格以以下 JSON 格式返回，不要包含任何 markdown 块或多余字符：
{
  "intent": "coupon" | "ticket" | "hotel" | "weekend" | "venue" | "chat"
}`;
...
3. 不得把模型知识伪装成实时商户、价格、评分、余票或优惠。酒店和票务会由独立实时工具处理；没有工具数据时只能给出通用规划建议，并明确建议用户另行核验。
4. 普通问答不要输出 <travel_deal>、<ticket_deal> 或任何预订卡片。只有拿到对应实时工具结果的专项节点才能输出带价格或链接的交易内容。
...
本节点没有商户实时排队、座位、门票或优惠数据。restaurantStatus 必须设为 null，不得编造余票、排队桌数、座位和 dpurl.cn 链接。
